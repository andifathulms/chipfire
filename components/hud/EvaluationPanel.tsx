  ...
  return (
    <div className="flex flex-col gap-xs text-sm">
          ...
          <tr className="border-b border-trace-hairline">
            ...
            {terms.map((_, player) => (
              <th key={player} scope="col" className={`${CELL} font-normal`}>
                ...
                <span className={styleFor(player).ink}>{playerName(player, locale)}</span>
              </th>
            ))}
          </tr>
        </thead>
      ...
      <p className="font-mono text-xs text-trace-faint">
        {COPY.weights[locale]}: orb {WEIGHTS.orb} · cell {WEIGHTS.cell} · position{' '}
        {WEIGHTS.position}×(5−mass) · vulnerability −{WEIGHTS.vulnerability}
      </p>

      <p className="max-w-measure text-xs text-trace-faint">{COPY.fair[locale]}</p>
    </div>
  )
}
