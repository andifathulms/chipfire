...
**The station at night.** A monitoring station's own display is emissive, and a detonation on an instrument is light rather than ink. Under `prefers-color-scheme: dark` the palette folds in on itself instead of a second one being invented: trace ink becomes the raised surface, chart stock becomes the ink, and the player colours lift until each carries words unaided — night ground `#171A18`, raised `#1F2421`, ink `#E8E4DC`, orange `#E2703A`, blue `#6FA8D6`, ochre `#D9AE49`, green `#6FAE92`. Only colour tokens are redefined; everything downstream inverts by construction. The mark's rust core never moves. Contrast is measured on both grounds before anything ships — the floor is 4.83:1.

**The afterglow.** A cascade leaves its path on the board until the next move: one intensity per cell, taken from the generation that cell last fired in, brightest where the fire reached last. It is the one thing the board says about how a position was reached rather than what it is, and it is a reading rather than an effect — it does not animate, so there is nothing for reduced motion to switch off and the still state is the complete state.
